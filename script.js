...
//fecchar o modal quando clicar fora
cartModal.addEventListener("click", function (event) {
    if (event.target === cartModal) {
        cartModal.style.display = "none"
    }
})
//fecher o modal quando clicar no botao fechar
...
function updatecartModal() {
    ...
    cart.forEach(item => {
       ...
       cartItemsCotainer.appendChild(cartitemElement)
...
//funçao para remover o intem do carrinho 
...
checkoutBtn.addEventListener("click", function(){
    ...
    const cartItems = cart.map((item) =>{
        return(
            `${item.name} Qauntidade: (${item.quantity})  | `
            // Preço: R$ ${item.price} 
        )
    }).join("")

    const message = encodeURIComponent(cartItems)
    const phone = "86999534606"
